...
sub post_blog_create {
    ...
    if ($main::user->{username} ne $author) {
        serve_error($client_socket, HTTP_RESPONSE::ERROR_401("You are not allowed to post this<br><a href=\"/ \">Return to index</a>"));
        return;
    }

    if ($is_announcement) {
        if ($main::user->{role} ne "admin") {
            serve_error($client_socket, HTTP_RESPONSE::ERROR_401("You are not allowed to post announcements<br><a href=\"/ \">Return to index</a>"));
            return;
        }
    }
    ...
    if (!$blog_name || !$blog_content) {
        serve_error($client_socket, HTTP_RESPONSE::ERROR_400("Title or content missing"));
        return;
    }
    ...
    if (-f $blog_path."/$title.json") {
        main::serve_error($client_socket, HTTP_RESPONSE::ERROR_409("Blog already exists"));
        return;
    }
    ...
    open(my $file, ">", $blog_path . "/$title.json") || die "Could not create blog file: $!";
    print $file $json;
    close $file;
    ...
    main::send_http_response($client_socket, $response);
}
